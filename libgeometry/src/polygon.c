...
int polygon_add_point(polygon_t *poly, point_t *p)
{
    ...
    if(poly->n_points >= 2)
    {
        segment_t new_seg, cur_seg;

        segment_init(&new_seg, &poly->points[poly->n_points - 1], p);

        for(unsigned int i=0; i < poly->n_points - 1; i++)
        {
            segment_init(&cur_seg, &poly->points[i], &poly->points[i+1]);

            if(segment_intersect(&cur_seg, &new_seg))
            {
                error("Adding point creates segment (%.2f, %.2f) -> (%.2f, %.2f) "
                      "which intersects with (%.2f, %.2f) -> (%.2f, %.2f)",
                      new_seg.p1->x, new_seg.p1->y, new_seg.p2->x, new_seg.p2->y,
                      cur_seg.p1->x, cur_seg.p1->y, cur_seg.p2->x, cur_seg.p2->y);
                return FAILURE;
            }
        }
    }
